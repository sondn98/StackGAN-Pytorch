...
class TextDataset(data.Dataset):
    ...
    def get_img(self, img_path):
        img = Image.open(img_path).convert('RGB')
        # TODO: ??? load_size = int(self.imsize * 76 / 64)
        load_size = int(self.imsize * 76 / 64)
        img = img.resize((load_size, load_size), PIL.Image.BILINEAR)
        if self.transform is not None:
            img = self.transform(img)
    ...
    #     bbox_path = join(data_dir, 'CUB_200_2011/bounding_boxes.txt')
    #     df_bounding_boxes = pd.read_csv(bbox_path,
    #                                     delim_whitespace=True,
    #                                     header=None).astype(int)
    #     #
    #     filepath = join(data_dir, 'CUB_200_2011/images.txt')
    #     df_filenames = \
    ...
    def load_embedding(self, data_dir):
        ...
        with open(join(data_dir, 'embedding.pkl'), 'rb') as f:
            embeddings = pickle.load(f)
            embeddings = np.array(embeddings)
            # embedding_shape = [embeddings.shape[-1]]
            print('embeddings: ', embeddings.shape)
    ...
    def load_class_id(self, data_dir, total_num):
        if isfile(data_dir + '/class_info.pickle'):
    ...
    def load_filenames(self, data_dir):
        filepath = join(data_dir, 'filenames.pickle')
    ...
    def __getitem__(self, index):
        ...
        #     bbox = self.bbox[key]
        #     data_dir = '%s/VN_CELEB' % self.data_dir
        # else:
        #     bbox = None
        #     data_dir = self.data_dir

        # captions = self.captions[key]
        img_name = join(self.data_dir, 'VN_CELEB/images', self.filenames[index])
        ...
        return img, self.embeddings[index]
